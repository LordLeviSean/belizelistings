/**
 * Browser client: decline viewing via server API with immediate push delivery.
 *
 ...
 */
export async function submitViewingDeclineViaApi(client, { viewingId, agentUserId, notes }) {
  ...
  const res = await fetch("/api/crm/viewing-decline", {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    return {
      data: null,
      error: {
        message: responseBody.error || responseBody.message || "Could not decline viewing.",
