...
import {
  ...
  applySeaFlowIntensityVarsToElement,
  getSeaFlowIntensityBootstrapScriptBody,
...
export function syncVisualModeDocument(state = readVisualModeState()) {
  ...
  applySeaFlowIntensityVarsToElement(root, normalized.seaFlowIntensity);
...
export function getVisualModeBootstrapScript() {
  const seaFlowBootstrap = getSeaFlowIntensityBootstrapScriptBody();
  return `(function(){try{${seaFlowBootstrap};var lp=localStorage.getItem("blz_live_palette_mode_v1")==="1";var pm=localStorage.getItem("blz_pulse_mode_v1")==="1";var sf=localStorage.getItem("blz_sea_flow_mode_v1")==="1";var si=localStorage.getItem("blz_sea_flow_intensity_v1");var el=document.documentElement;el.setAttribute("data-live-palette",lp?"true":"false");el.setAttribute("data-pulse-mode",pm?"true":"false");el.setAttribute("data-sea-flow",sf?"on":"off");blzApplySeaFlowVars(el,si!=null&&si!==""?si:${SEA_FLOW_INTENSITY_DEFAULT});var rm=window.matchMedia("(prefers-reduced-motion: reduce)").matches;el.setAttribute("data-reduced-motion",rm?"true":"false");}catch(e){}})();`;
}
