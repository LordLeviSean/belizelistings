import {
  ...
  PERMANENT_USER_DELETE_SERVICE_ROLE_REQUIRED_MESSAGE,
  ...
  permanentUserDeleteServiceRoleRequiredError,
...
describe("userPermanentDelete", () => {
  ...
  test("mapPermanentUserDeleteApiError maps missing service role configuration", () => {
    const err = mapPermanentUserDeleteApiError({
      code: SUPABASE_SERVICE_ROLE_CONFIG_MISSING_CODE,
      error: formatPermanentUserDeleteServerConfigError(["SUPABASE_SERVICE_ROLE_KEY"]),
    });
    expect(err.message).toBe(PERMANENT_USER_DELETE_SERVICE_ROLE_REQUIRED_MESSAGE);
  });

  test("mapPermanentUserDeleteApiError maps legacy service role configuration message", () => {
    const err = mapPermanentUserDeleteApiError({
  ...
  test("formatPermanentUserDeleteServerConfigError names missing env vars", () => {
    expect(formatPermanentUserDeleteServerConfigError(["SUPABASE_SERVICE_ROLE_KEY"])).toBe(
      PERMANENT_USER_DELETE_SERVICE_ROLE_REQUIRED_MESSAGE
    );
    expect(formatPermanentUserDeleteServerConfigError(["NEXT_PUBLIC_SUPABASE_URL"])).toContain(
    ...
    ).toContain("NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY");
  });

  test("permanentUserDeleteServiceRoleRequiredError returns actionable message", () => {
    expect(permanentUserDeleteServiceRoleRequiredError().message).toBe(
      PERMANENT_USER_DELETE_SERVICE_ROLE_REQUIRED_MESSAGE
    );
  });
  ...
  test("permanentlyDeleteUserViaApi maps missing service role configuration", async () => {
    global.fetch = jest.fn().mockResolvedValue({
      ok: false,
      status: 503,
      json: async () => ({
        code: SUPABASE_SERVICE_ROLE_CONFIG_MISSING_CODE,
        error: PERMANENT_USER_DELETE_SERVICE_ROLE_REQUIRED_MESSAGE,
      }),
    });

    const result = await permanentlyDeleteUserViaApi({ userId: "user-uuid" });
    expect(result.ok).toBe(false);
    expect(result.error.message).toBe(PERMANENT_USER_DELETE_SERVICE_ROLE_REQUIRED_MESSAGE);
  });
