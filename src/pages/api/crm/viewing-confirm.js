...
import { performConfirmViewing } from "@/lib/crm/viewingMutations";
...
export default async function handler(req, res) {
  ...
  if (!url || !serviceRole || !anonKey) {
    return res.status(503).json({ error: "Viewing confirm API is not configured." });
  }
  ...
  if (String(viewing.agent_user_id) !== String(user.id)) {
    return res.status(403).json({
      code: "forbidden",
      error: "You cannot confirm this viewing request.",
    });
  }
  ...
  if (viewing.status !== VIEWING_STATUS.PENDING) {
    return res.status(409).json({
      ...
      error: "Only pending viewing requests can be confirmed.",
    });
  }

  const result = await performConfirmViewing(adminClient, {
    viewingId,
    agentUserId: user.id,
    notes,
  });
  ...
  if (result.error) {
    const message = result.error.message || "Could not confirm viewing.";
    const status = result.error.code === "PGRST116" ? 409 : 400;
    return res.status(status).json({
      code: status === 409 ? "viewing_not_pending" : "confirm_failed",
      error: message,
    });
  }
  ...
  return res.status(200).json({
    ...
    data: {
      ...
      status: result.data?.status ?? VIEWING_STATUS.CONFIRMED,
      confirmed_at: result.data?.confirmed_at ?? null,
