...
import { performBuyerReply } from "@/lib/crm/conversationMutations";
...
export default async function handler(req, res) {
  ...
  if (!url || !serviceRole || !anonKey) {
    return res.status(503).json({ error: "Buyer reply API is not configured." });
  }
  ...
  const senderName = body.senderName ?? body.sender_name ?? null;
  ...
  if (String(conversation.buyer_id) !== String(user.id)) {
    return res.status(403).json({ code: "forbidden", error: "You cannot reply in this conversation." });
  }

  const result = await performBuyerReply(adminClient, {
    conversationId,
    buyerUserId: user.id,
    body: replyBody,
    listingId,
  ...
  if (result.error) {
    const message = result.error.message || "Could not send message.";
